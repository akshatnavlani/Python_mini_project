...
def modify_expenses(expense_id, amount, description, date):
    ...
    try:
        cursor.execute("UPDATE expenses SET amount=?, description=?, date=? WHERE id=?",
...
def display_expenses(expenses, title):
    if expenses:
        st.header(title)
        df = pd.DataFrame(expenses, columns=["Expense ID", "User ID", "Amount", "Description", "Date", "Category"])
        st.dataframe(df)
    else:
