...
def modify_expenses(expense_id, expenses, description, date):
    conn = sqlite3.connect("user_data.db")
    cursor = conn.cursor()

    try:
        cursor.execute("UPDATE expenses SET income=?, expenses=?, description=?, date=? WHERE id=?",
                       (expenses, description, date, expense_id))
        conn.commit()
        conn.close()
...
def display_expenses(expenses, title):
    if expenses:
        st.header(title)
        df = pd.DataFrame(expenses, columns=["Expense ID", "User ID", "Expenses", "Description", "Date", "Category"])
        st.dataframe(df)
    else:
